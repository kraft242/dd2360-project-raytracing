...
__device__ vec3 random_in_unit_disk(curandState *local_rand_state)
{
    ...
    do
    {
        p = FpDataType(2.0f) * vec3(curand_uniform(local_rand_state), curand_uniform(local_rand_state), 0) - vec3(1, 1, 0);
    } while (d_dot(p, p) >= FpDataType(1.0f));
    return p;
}
...
public:
    ...
    { // vfov is top to bottom in degrees
        lens_radius = aperture / FpDataType(2.0f);
        FpDataType theta = vfov * ((FpDataType)M_PI) / FpDataType(180.0f);
        FpDataType half_height = d_tan(theta / FpDataType(2.0f));
        ...
        horizontal = FpDataType(2.0f) * half_width * focus_dist * u;
        vertical = FpDataType(2.0f) * half_height * focus_dist * v;
    }
